...
if ($landing_id < 1) {
    alert('랜딩을 선택하세요.', SF_MANAGER_URL . '/landing/notice_form.php' . ($id ? '?id=' . $id : ''));
}
if ($title === '') {
    alert('공지 제목을 입력하세요.', SF_MANAGER_URL . '/landing/notice_form.php' . ($id ? '?id=' . $id : ''));
}
if ($content === '') {
    alert('공지 내용을 입력하세요.', SF_MANAGER_URL . '/landing/notice_form.php' . ($id ? '?id=' . $id : ''));
}
if ($is_active !== 'N') {
    $is_active = 'Y';
}
...
if ($id) {
    ...
    alert('공지 항목을 수정했습니다.', SF_MANAGER_URL . '/landing/notice_form.php?id=' . $id);
}

sql_query(" insert into {$table} set {$set_sql}, created_at = '" . G5_TIME_YMDHIS . "' ");
$new_id = sql_insert_id();
alert('공지 항목을 등록했습니다.', SF_MANAGER_URL . '/landing/notice_form.php?id=' . $new_id);