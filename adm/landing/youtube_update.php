...
if ($landing_id < 1) {
    alert('랜딩을 선택하세요.', SF_MANAGER_URL . '/landing/youtube_form.php' . ($id ? '?id=' . $id : ''));
}
if ($title === '') {
    alert('제목을 입력하세요.', SF_MANAGER_URL . '/landing/youtube_form.php' . ($id ? '?id=' . $id : ''));
}
if ($youtube_url === '') {
    alert('YouTube URL을 입력하세요.', SF_MANAGER_URL . '/landing/youtube_form.php' . ($id ? '?id=' . $id : ''));
}

$video_id = sf_extract_youtube_id($youtube_url);
if ($video_id === '') {
    alert('유효한 YouTube URL이 아닙니다.', SF_MANAGER_URL . '/landing/youtube_form.php' . ($id ? '?id=' . $id : ''));
}
...
if ($id) {
    ...
    alert('유튜브 항목을 수정했습니다.', SF_MANAGER_URL . '/landing/youtube_form.php?id=' . $id);
}

sql_query(" insert into {$table} set {$set_sql}, created_at = '" . G5_TIME_YMDHIS . "' ");
$new_id = sql_insert_id();
alert('유튜브 항목을 등록했습니다.', SF_MANAGER_URL . '/landing/youtube_form.php?id=' . $new_id);