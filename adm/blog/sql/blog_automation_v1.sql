...
-- 테이블명은 blog_ 네임스페이스로 통일한다 — 원래 BLOG_AUTOMATION_DATA_MODEL.md 6절은
-- 서비스 중립 공통 테이블에 blog_ 접두어를 금지했으나, 운영 DB에 아직 테이블이 하나도
-- 없는 시점에 PM 지시로 이 방침을 뒤집고 전부 blog_ 접두어를 붙이기로 확정했다(2026-07-29).
...
-- 1. 광고주 (공통, 최소 업체정보)
CREATE TABLE IF NOT EXISTS `{prefix}blog_advertisers` (
...
-- 2. 발행 사이트 (공통)
CREATE TABLE IF NOT EXISTS `{prefix}blog_sites` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_sites_advertiser` FOREIGN KEY (`advertiser_id`) REFERENCES `{prefix}blog_advertisers` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 3. 사이트별 발행 채널 인증 정보 (WordPress Application Password 등) — 값은 암호화 저장, 화면엔 masked_hint만 노출
CREATE TABLE IF NOT EXISTS `{prefix}blog_site_credentials` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_credentials_site` FOREIGN KEY (`site_id`) REFERENCES `{prefix}blog_sites` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 4. AI 공급자 설정 (최소 — API 키는 암호화 저장, masked_hint만 화면 노출)
CREATE TABLE IF NOT EXISTS `{prefix}blog_ai_providers` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `{prefix}blog_content_projects` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  KEY `idx_advertiser_id` (`advertiser_id`),
  KEY `idx_status` (`status`),
  CONSTRAINT `fk_projects_advertiser` FOREIGN KEY (`advertiser_id`) REFERENCES `{prefix}blog_advertisers` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `fk_projects_site` FOREIGN KEY (`primary_site_id`) REFERENCES `{prefix}blog_sites` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 6. 프로젝트별 키워드 (그룹 분류 + 잠금)
CREATE TABLE IF NOT EXISTS `{prefix}blog_content_keywords` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_keywords_project` FOREIGN KEY (`project_id`) REFERENCES `{prefix}blog_content_projects` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `{prefix}blog_posts` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_project_id` (`project_id`),
  CONSTRAINT `fk_posts_project` FOREIGN KEY (`project_id`) REFERENCES `{prefix}blog_content_projects` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 8. 사이트별 발행 인스턴스 (1개 포스팅 : N개 사이트) — 제목/본문/이미지/예약/상태/재시도를 사이트마다 별도 저장
CREATE TABLE IF NOT EXISTS `{prefix}blog_post_targets` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_targets_post` FOREIGN KEY (`post_id`) REFERENCES `{prefix}blog_posts` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `fk_targets_site` FOREIGN KEY (`site_id`) REFERENCES `{prefix}blog_sites` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `{prefix}blog_publish_jobs` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  KEY `idx_status` (`status`),
  CONSTRAINT `fk_jobs_target` FOREIGN KEY (`post_target_id`) REFERENCES `{prefix}blog_post_targets` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 10. 발행 시도 이력 (성공/실패 각 시도 기록 — response_message에 절대 비밀정보 기록 금지)
CREATE TABLE IF NOT EXISTS `{prefix}blog_publish_attempts` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_attempts_job` FOREIGN KEY (`publish_job_id`) REFERENCES `{prefix}blog_publish_jobs` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 11. 상태 변경 활동 로그 (승인 상태머신 감사 추적용)
CREATE TABLE IF NOT EXISTS `{prefix}blog_content_activity_logs` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `project_id` INT UNSIGNED NOT NULL,
  ...
  PRIMARY KEY (`id`),
  KEY `idx_project_id` (`project_id`),
  CONSTRAINT `fk_logs_project` FOREIGN KEY (`project_id`) REFERENCES `{prefix}blog_content_projects` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
