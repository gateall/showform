...
function social_build_provider_config($provider){
    ...
    // 디버깅 목적으로 항상 디버그 로그를 활성화하고 /data/session/social_debug.log에 기록합니다.
    $setting['debug_mode'] = true;
    $setting['debug_file'] = G5_DATA_PATH.'/session/social_debug.log';
...
function social_check_login_before($p_service=''){
    ...
    if( $provider_name ){
        ...
        catch( Exception $e )
        {
            ...
            $get_error .= "<br><br><b>[디버그 상세 오류 내용]</b><br>" . nl2br(htmlspecialchars($e->getMessage())) . "<br><br><b>[에러 코드]</b>: " . htmlspecialchars($e->getCode());
