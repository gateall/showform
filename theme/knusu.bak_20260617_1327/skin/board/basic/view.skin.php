<style>
    ...
    @media(max-width:768px) {
        ...
        .estimate-view-table,
        .estimate-view-table tbody,
        .estimate-view-table tr,
        .estimate-view-table th,
        .estimate-view-table td {
            display: block;
            width: 100%;
...
<article id="bo_v" style="width:<?php echo $width; ?>">
    <header>
        <h2 id="bo_v_title">
            ...
            if ($status_text === '견적확인')
                $status_color = '#f39c12';
            elseif ($status_text === '견적제출')
                $status_color = '#073567';
            if ($bo_table == 'inquiry') {
                echo '<span style="display:inline-block; padding:3px 10px; font-size:12px; color:#fff; background-color:' . $status_color . '; border-radius:3px; vertical-align:middle; margin-right:10px;">' . $status_text . '</span>';
            }
            ?>
            <?php if ($category_name) { ?>
                <span class="bo_v_cate">
                    <?php echo $view['ca_name']; // 분류 출력 ?? ?>
                </span>
            <?php } ?>
            <span class="bo_v_tit">
                <?php
                echo cut_str(get_text($view['wr_subject']), 70); // 湲?쒕ぉ 異쒕젰
                ?>
            </span>
        </h2>
    </header>
            ...
            <ul class="btn_bo_user bo_v_com">
                ...
                <?php if ($reply_href) { ?>
                    <li><a href="<?php echo $reply_href ?>" class="btn_b01 btn" title="답변"><i class="fa fa-reply"
                                aria-hidden="true"></i><span class="sound_only">답변</span></a></li>
                <?php } ?>
                <?php if ($write_href) { ?>
                    <li><a href="<?php echo $write_href ?>" class="btn_b01 btn" title="湲?곌린"><i class="fa fa-pencil"
                                aria-hidden="true"></i><span class="sound_only">湲?곌린</span></a></li>
                <?php } ?>
                <?php if ($update_href || $delete_href || $copy_href || $move_href || $search_href) { ?>
                    <li>
                        ...
                        <ul class="more_opt is_view_btn">
                            ...
                            <?php } ?>
                            <?php if ($search_href) { ?>
                                <li><a href="<?php echo $search_href ?>">寃??i class="fa fa-search" aria-hidden="true"></i></a>
                                </li>
                            <?php } ?>
                        </ul>
                    </li>
    ...
    <section id="bo_v_atc">
        ...
        <?php //echo $view['rich_content']; // {?대?吏:0} 怨?媛숈? 肄붾뱶瑜??ъ슜??寃쎌슦 ?>
        <!-- } 본문 내용 끝 -->
    ...
    <?php if ($prev_href || $next_href) { ?>
        <ul class="bo_v_nb">
            <?php if ($prev_href) { ?>
                <li class="btn_prv"><span class="nb_tit"><i class="fa fa-chevron-up" aria-hidden="true"></i> ?댁쟾湲</span><a
            ...
            <?php } ?>
            <?php if ($next_href) { ?>
                <li class="btn_next"><span class="nb_tit"><i class="fa fa-chevron-down" aria-hidden="true"></i> ?ㅼ쓬湲</span><a
    ...
    <?php if ($bo_table == 'inquiry' && $is_admin) { ?>
        <div style="text-align:center; margin:30px 0;">
            <a href="<?php echo G5_BBS_URL; ?>/write.php?w=u&bo_table=<?php echo $bo_table; ?>&wr_id=<?php echo $wr_id; ?>&status=견적확인"
                class="btn btn_b02" style="font-size:16px; padding:10px 20px;">견적확인</a>
            <a href="<?php echo G5_BBS_URL; ?>/write.php?w=u&bo_table=<?php echo $bo_table; ?>&wr_id=<?php echo $wr_id; ?>&status=견적접수"
                class="btn btn_b01" style="font-size:16px; padding:10px 20px;">견적접수</a>
            <a href="<?php echo G5_BBS_URL; ?>/write.php?w=u&bo_table=<?php echo $bo_table; ?>&wr_id=<?php echo $wr_id; ?>&status=견적제출"
                class="btn btn_admin" style="font-size:16px; padding:10px 20px;">견적제출</a>
        </div>
    <?php } ?>
    ...
    <?php if ($board['bo_download_point'] < 0) { ?>
        $(function () {
            $("a.view_file_download").click(function () {
                ...
                var msg = "?뚯씪회 다운로드 ?섏떆硫??ъ씤?멸? 李④컧(<?php echo number_format($board['bo_download_point']) ?>???⑸땲??\n\n?ъ씤?몃뒗 寃뚯떆臾쇰떦 ?쒕쾲留?李④컧?섎ŉ ?ㅼ쓬???ㅼ떆 ?ㅼ슫濡쒕뱶 ?섏뀛??以묐났?섏뿬 李④컧?섏? ?딆뒿?덈떎.\n\n洹몃옒회 다운로드 ?섏떆寃좎뒿?덇퉴?";
