    ...
    <div class="login-card">
        ...
        <form name="flogin" action="<?php echo $login_action_url ?>" onsubmit="return flogin_submit(this);" method="post" id="flogin" class="login-form">
            ...
            <div id="sns_login" class="login-sns sns-wrap-32 sns-wrap-over">
                <h3>소셜계정으로 로그인</h3>
                <div class="sns-wrap">
                    <a href="<?php echo $kakao_login_url; ?>" class="sns-icon social_link sns-kakao" title="카카오">
                        <span class="ico"></span>
...
jQuery(function($){
    $(".sns-wrap").on("click", "a.social_link", function(e){
        ...
        if(!newWin || newWin.closed || typeof newWin.closed == 'undefined') {
            alert('브라우저에서 팝업창이 차단되어 있습니다. 팝업 차단을 해제한 후 다시 시도해 주세요.');
        }
...
#mb_login .login-card{background:#fff;border:1px solid #e7ebef;border-radius:24px;box-shadow:0 18px 50px rgba(16,24,40,.08);padding:24px}
...
#mb_login .sns-icon.sns-kakao{display:flex;align-items:center;justify-content:center;gap:10px;width:100%;min-height:54px;border-radius:14px;background:#fee500;color:#191919;font-weight:800;text-decoration:none}
#mb_login .sns-icon .ico{width:24px;height:24px;background:url('<?php echo G5_IMG_URL; ?>/kakao.png') no-repeat center;background-size:contain;flex-shrink:0}
#mb_login .sns-icon .txt i{font-style:normal}
...
#mb_login .login-field .frm_input{width:100%;height:52px;border-radius:14px;border:1px solid #d0d5dd;padding:0 16px;font-size:16px;background:#fff}
#mb_login .login-password-wrap{display:flex;gap:10px;align-items:center}
#mb_login .login-password-wrap .frm_input{flex:1}
#mb_login .login-password-toggle{height:52px;padding:0 14px;border:1px solid #d0d5dd;border-radius:14px;background:#fff;color:#344054;font-weight:700}
#mb_login .login-submit{width:100%;height:56px;border:0;border-radius:16px;background:linear-gradient(135deg,#b8e986 0%,#7bc96f 100%);color:#fff;font-size:17px;font-weight:800;margin-top:4px}
...
@media (max-width:768px){
    ...
    #mb_login .login-field .frm_input,
    #mb_login .login-password-toggle,
    #mb_login .login-submit{height:48px;font-size:15px}
    #mb_login .login-password-wrap{gap:8px}
    #mb_login .login-links{flex-direction:column;align-items:flex-start;gap:8px}
    #mb_login #sns_login{display:none !important}
}
