<?php
if (!defined('_GNUBOARD_')) exit; // 揶쏆뮆????륁뵠筌왖 ?臾롫젏 ?븍뜃?

// ?醫뤾문?????곗쨮 ?紐낅퉸 ????뱁뒄疫꿸퀗? 揶쎛癰궰?怨몄몵嚥?癰궰??
$colspan = 5;
...
// add_stylesheet('css ?닌됎?, ?곗뮆???뽮퐣); ??ъ쁽揶쎛 ?臾믪뱽 ??롮쨯 ?믪눘? ?곗뮆???
...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <?php if ($is_category) { ?>
    <nav id="bo_cate">
        <h2><?php echo $board['bo_subject'] ?> 燁삳똾?믤⑥쥓??/h2>
        <ul id="bo_cate_ul">
    ...
    <div id="bo_btn_top">
        ...
        <ul class="btn_bo_user">
        	<?php if ($admin_href) { ?><li><a href="<?php echo $admin_href ?>" class="btn_admin btn" title="?온?귐딆쁽"><i class="fa fa-cog fa-spin fa-fw"></i><span class="sound_only">?온?귐딆쁽</span></a></li><?php } ?>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> <?php echo $lt_class ?>">
            ...
            <td class="td_subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '0'; ?>px">
                ...
                if ($bo_table == 'inquiry') {
                    $status_text = isset($list[$i]['wr_8']) && trim((string)$list[$i]['wr_8']) !== '' ? trim((string)$list[$i]['wr_8']) : '寃ъ쟻?묒닔';
                    $status_color = '#01b6eb';
                    if ($status_text === '寃ъ쟻?뺤씤') $status_color = '#f39c12';
                    elseif ($status_text === '寃ъ쟻?쒖텧') $status_color = '#073567';
    ...
    <div class="bo_sch_wrap">
        <fieldset class="bo_sch">
            ...
            <label for="stx" class="sound_only">寃?됱뼱<strong class="sound_only"> 媛쒖닔</strong></label>
            <div class="sch_bar">
                <input type="text" name="stx" value="<?php echo stripslashes($stx) ?>" required id="stx" class="sch_input" size="25" maxlength="20" placeholder=" 寃?됱뼱瑜??낅젰??二쇱꽭??>
