...
?>
<style>
/* 반응형 적용을 위한 래퍼 기본 스타일 */
.estimate-origin-wrap {
    width: 100%;
    max-width: 1250px;
    margin: 0 auto;
    overflow: hidden;
...
@media(max-width:768px){
    ...
    .ms_inquiry .mi_tel img{
        display:none;
    }
    
    /* 체크박스 레이아웃 깨짐 방지 */
    .ms_inquiry li.mi_tel > div {
        width: 100% !important;
...
/* 모바일 견적접수확인 카드화 및 반응형 */
@media (max-width:768px){
    .mb_con{width:100%!important; padding:0 12px!important; box-sizing:border-box!important;}
...
<section class="estimate-origin-wrap" id="estimate">
    <div class="estimate-origin-inner">
        <!-- 1. 실시간 견적문의 form 영역 -->
        <div style="width:calc(50% - 15px); min-height:550px; text-align:center; padding-top:30px; background-color:#fff; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.05); box-sizing:border-box;">
            <form name="fwrite" id="fwrite" action="/bbs/form_insert.php" onsubmit="return fwrite_submit_x(this);" method="post" enctype="multipart/form-data" autocomplete="off">
                    ...
                    <div class="ms_inquiry ms_contents">
                        <ul style="padding:0;">
                            ...
                            <li class="mi_tel">
                                ...
                                <input type="text" name="wr_3" placeholder="출발지주소" required="required" style="width:230px;" onclick="openDaumPostcode();">
                                <input type="hidden" name="wr_6">
                                <input type="text" name="wr_3_2" placeholder="평수" required="required" style="width:64px; margin-left:3px;">
                                <input type="text" name="wr_3_3" placeholder="층수" required="required" style="width:64px; margin-left:3px;">
                                <div class="elevator-check-wrap">
                                    <span>출발지 엘리베이터</span>
                                    <label><input type="checkbox" name="wr_3_elevator" value="있음" class="elevator-check">있음</label> <label><input type="checkbox" name="wr_3_elevator" value="없음" class="elevator-check">없음</label>
                                </div>
                            </li>
                            <li class="mi_tel">
                                <label class="three">도착지정보</label>
                                <input type="text" name="wr_4" placeholder="도착지주소" required="required" style="width:230px;" onclick="openDaumPostcode1();">
                                <input type="hidden" name="wr_7">
                                <input type="text" name="wr_4_2" placeholder="평수" required="required" style="width:64px; margin-left:3px;">
                                <input type="text" name="wr_4_3" placeholder="층수" required="required" style="width:64px; margin-left:3px;">
                                <div class="elevator-check-wrap">
                                    <span>도착지 엘리베이터</span>
                                    <label><input type="checkbox" name="wr_4_elevator" value="있음" class="elevator-check">있음</label> <label><input type="checkbox" name="wr_4_elevator" value="없음" class="elevator-check">없음</label>
                                </div>
                            </li>
                            <li class="mi_tel">
                                <label class="three">포함될가구</label>
                                <div class="furniture-check-wrap">
                                    <label><input type="checkbox" name="wr_5[]" value="에어컨">에어컨</label> <label><input type="checkbox" name="wr_5[]" value="피아노">피아노</label> <label><input type="checkbox" name="wr_5[]" value="돌침대">돌침대</label>
                                    <label><input type="checkbox" name="wr_5[]" value="벽걸이TV">벽걸이TV</label> <label><input type="checkbox" name="wr_5[]" value="조립식장롱">조립식장롱</label>
                                </div>
                            </li>
                        ...
                        <div class="mi_agree notoSans" style="margin-top:20px;">
                            ...
                            <div class="mi_check">
                                <input type="checkbox" id="agree_chk_latest" required="required">
        ...
        <div style="width:calc(50% - 15px); min-height:550px; text-align:center; padding-top:30px; background-color:#fff; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.05); box-sizing:border-box;">
            <div class="ms_box02 ms_box">   
                <h2 style="font-size:30px; line-height:40px; font-weight:900; letter-spacing:-0.05em; margin-bottom:20px;"><i class="fa fa-file-text-o"></i> 견적접수확인</h2>
...
<script>
// 5. fwrite_submit_x() 확인 함수
function fwrite_submit_x(f){
...
$(document).ready(function() {
    ...
    $("#wr_2").datepicker({ 
        ...
        showOn: 'both',
        buttonImage: '/theme/landing/img/btn001.gif',
        buttonImageOnly: true
    });
});

// 4. 다음 우편번호 스크립트 호출 함수
function openDaumPostcode() {	    
    daum.postcode.load(function(){
       new daum.Postcode({
          oncomplete: function(data) {					
             document.fwrite.wr_3.value = data.address + " " + data.buildingName;
             document.fwrite.wr_3.focus();
             document.fwrite.wr_6.value = data.address;
          }
       }).open();
    });
}
function openDaumPostcode1() {	    
    daum.postcode.load(function(){
       new daum.Postcode({
          oncomplete: function(data) {					
             document.fwrite.wr_4.value = data.address + " " + data.buildingName;
             document.fwrite.wr_4.focus();
             document.fwrite.wr_7.value = data.address;
          }
       }).open();
    });
}
