...
// E-mail 주소를 아이디로 사용하기 위한 mb_id 필드 크기 마이그레이션 (1회 실행)
if ($w == '') {
    sql_query("ALTER TABLE {$g5['member_table']} MODIFY mb_id VARCHAR(100) NOT NULL DEFAULT ''", false);
}
...
/* 이메일 기반 회원아이디 자동 생성 */
if ($w == '' && isset($_POST['mb_email']) && $_POST['mb_email']) {
    $email_id = explode('@', $_POST['mb_email']);
    $make_id = preg_replace('/[^a-zA-Z0-9_]/', '_', $email_id[0]);

    if (!$make_id) {
        $make_id = 'user';
    }

    $mb_id = $make_id . '_' . rand(10000, 99999);
    $_POST['mb_id'] = $mb_id;
...
/* 실명 기반 닉네임 자동 생성 */
if ($w == '' && isset($_POST['mb_name']) && $_POST['mb_name']) {
    $base_nick = preg_replace(
        '/[^a-zA-Z0-9가-힣_]/u',
        '',
        trim($_POST['mb_name'])
    );
    ...
    /* 랜덤 숫자 부여 */
    $mb_nick = $base_nick . rand(1000, 9999);

    /* POST 값 동기화 */
    $_POST['mb_nick'] = $mb_nick;
