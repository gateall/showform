...
$contact_board_tables = array('inquiry', 'qa');
$is_contact_board = (isset($_POST['bo_table']) && in_array($_POST['bo_table'], $contact_board_tables, true));
$is_contact_new = ($is_contact_board && (!isset($_POST['w']) || $_POST['w'] === '' || $_POST['w'] === 'r'));
$is_online_inquiry = ($is_contact_new && isset($_POST['wr_10']) && $_POST['wr_10'] === 'online_page');
$posted_wr_email = isset($_POST['wr_email']) ? trim($_POST['wr_email']) : '';
if ($posted_wr_email === '' && isset($_POST['wr_1'])) {
    $posted_wr_email = trim($_POST['wr_1']);
}
$skip_inquiry_captcha = false;
...
if ($wr_subject == '' && !$is_online_inquiry) {
...
if ($wr_content == '' && !$is_online_inquiry) {
...
// inquiry 공통 접수 검증/정리
if ($is_contact_new && isset($_POST['wr_10'])) {
    $inquiry_source = isset($wr_10) ? trim($wr_10) : '';
    $allowed_sources = array('footer_contact', 'online_page');
    if (!in_array($inquiry_source, $allowed_sources, true)) {
        alert('올바르지 않은 접수 경로입니다.');
    }

    $skip_inquiry_captcha = true;

    $honeypot = isset($_POST['contact_website']) ? trim($_POST['contact_website']) : '';
    if ($honeypot !== '') {
        alert('비정상 접수로 차단되었습니다.');
    }

    $form_ts = isset($_POST['form_ts']) ? (int)$_POST['form_ts'] : 0;
    if ($form_ts <= 0 || (G5_SERVER_TIME - $form_ts) < 3 || (G5_SERVER_TIME - $form_ts) > 7200) {
        alert('접수 시간이 올바르지 않습니다. 다시 시도해 주세요.');
    }

    $agree = isset($_POST['agree']) ? trim((string)$_POST['agree']) : '';
    if ($agree !== '1' && strtolower($agree) !== 'on') {
        alert('媛쒖씤?뺣낫 ?섏쭛 諛??댁슜?먯뿉 ?숈쓽??二쇱꽭??');
    }

    $wr_2 = preg_replace('/[^0-9]/', '', $wr_2);
    if (strlen($wr_2) < 9 || strlen($wr_2) > 11) {
        alert('연락처를 올바르게 입력해 주세요.');
    }
    if (strlen($wr_2) === 11) {
        $wr_2 = preg_replace('/(\d{3})(\d{4})(\d{4})/', '$1-$2-$3', $wr_2);
    } else if (strlen($wr_2) === 10) {
        $wr_2 = preg_replace('/(\d{2,3})(\d{3,4})(\d{4})/', '$1-$2-$3', $wr_2);
    }
    if ($inquiry_source !== 'online_page' && strlen($wr_content) < 5) {
        alert('문의내용을 5자 이상 입력해 주세요.');
    }
}

if ($is_contact_new && isset($inquiry_source) && $inquiry_source === 'online_page') {
    if ($wr_subject === '') {
        $wr_subject = '온라인상담 문의';
    }
    if ($wr_content === '') {
...
$is_use_captcha = ((($board['bo_use_captcha'] && $w !== 'u') || $is_guest) && !$is_admin && !$skip_inquiry_captcha) ? 1 : 0;
...
if ((!isset($_POST['wr_subject']) || !trim($_POST['wr_subject'])) && !$is_online_inquiry)
    alert('제목을 입력하여 주십시오.');
...
if ($w == '' || $w == 'r') {
    ...
    if ($member['mb_id']) {
        ...
        $wr_email = get_email_address($posted_wr_email);
    ...
    if ($member['mb_id']) {
        ...
        if ($member['mb_id'] === $wr['mb_id']) {
            ...
            if(isset($posted_wr_email) && $posted_wr_email)
                $wr_email = get_email_address($posted_wr_email);
            else
...
// inquiry 접수 메일: 저장 완료 후 관리자에게 별도 발송
if ($is_contact_new && isset($wr_10) && in_array($wr_10, array('footer_contact', 'online_page'), true) && !($w == 'u' || $w == 'cu')) {
    include_once(G5_LIB_PATH.'/mailer.lib.php');

    $source_title_map = array(
        'footer_contact' => '하단 접수창',
        'online_page' => '온라인상담 페이지'
    );
    $source_key = isset($wr_10) ? trim($wr_10) : '';
    $source_title = isset($source_title_map[$source_key]) ? $source_title_map[$source_key] : '문의폼';
    $site_title = trim((string)(isset($config['cf_title']) ? $config['cf_title'] : ''));
    if ($site_title === '') {
        $site_title = '인터넷2424';
    }
    $mail_subject = '['.$site_title.'] 문의 접수 - '.$source_title;

    $mail_from_name = $config['cf_admin_email_name'] ? $config['cf_admin_email_name'] : $site_title;
    $mail_from_email = $config['cf_admin_email'] ? $config['cf_admin_email'] : '';
    $mail_to = $config['cf_admin_email'] ? $config['cf_admin_email'] : '';

    if (!$mail_to) {
        $super_admin = get_admin('super');
        if (!empty($super_admin['mb_email'])) {
            $mail_to = $super_admin['mb_email'];
        }
    }

    if ($mail_from_email && $mail_to) {
        $safe_name = get_text(stripslashes($wr_name));
        $safe_phone = get_text(stripslashes($wr_2));
        $safe_subject = get_text(stripslashes($wr_subject));
        $safe_content = nl2br(get_text(stripslashes($wr_content), 1));
        $safe_source = get_text($source_key);
        $safe_ip = get_text($_SERVER['REMOTE_ADDR']);

        $mail_content = '';
        $mail_content .= '<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.7;">';
        $mail_content .= '<h2 style="margin:0 0 16px;">문의 접수 알림</h2>';
        $mail_content .= '<p><strong>접수경로:</strong> '.$source_title.' ('.$safe_source.')</p>';
        $mail_content .= '<p><strong>이름:</strong> '.$safe_name.'</p>';
        $mail_content .= '<p><strong>연락처:</strong> '.$safe_phone.'</p>';
        $mail_content .= '<p><strong>제목:</strong> '.$safe_subject.'</p>';
        $mail_content .= '<p><strong>접수일시:</strong> '.G5_TIME_YMDHIS.'</p>';
        $mail_content .= '<p><strong>접속IP:</strong> '.$safe_ip.'</p>';
        $mail_content .= '<hr style="margin:16px 0;border:0;border-top:1px solid #ddd;">';
        $mail_content .= '<p><strong>문의내용</strong><br>'.$safe_content.'</p>';
        $mail_content .= '</div>';

        // 저장 성공 후 메일 발송 (실패해도 저장은 유지)
        @mailer($mail_from_name, $mail_from_email, $mail_to, $mail_subject, $mail_content, 1);
    }
}

// 게시판 기본 메일발송 사용 (수정글은 발송하지 않음, inquiry는 위 커스텀 메일만 사용)
if (!(isset($wr_10) && in_array($wr_10, array('footer_contact', 'online_page'), true)) && !($w == 'u' || $w == 'cu') && $config['cf_email_use'] && $board['bo_use_email']) {
...
if (isset($wr_10) && in_array($wr_10, array('footer_contact', 'online_page'), true) && ($w === '' || $w === 'r')) {
    alert('신청되었습니다.', G5_URL.'/');
}
