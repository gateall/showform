...
<article id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <header>
        ...
        .estimate-status-box{margin:20px 0;padding:15px;border:1px solid #ddd;background:#f8f8f8;font-size:16px;text-align:center;}


        .estimate-status-box strong{color:#dc2626;}
        ...
        <div class="estimate-status-box">
            ...
            <strong><?php echo $view['wr_10'] ? $view['wr_10'] : '접수대기'; ?></strong>
    ...
    <tbody>
        ...
        <?php if ($bo_table === 'isa_inquiry') { ?>

        <?php

        $status_text = isset($view['wr_8']) && trim((string)$view['wr_8']) !== '' ? trim((string)$view['wr_8']) : '견적접수';

        $status_bg = '#01b6eb';

        if ($status_text === '견적검토중') {

            $status_bg = '#f6a800';

        } else if ($status_text === '견적확인') {
        ...
        <div style="margin:20px 0 10px; padding:14px; border:1px solid #e5e5e5; background:#fafafa; border-radius:8px;">

            <div style="margin-bottom:10px;">

                <span class="bbok" style="display:inline-block; background-color:<?php echo $status_bg; ?>;"><?php echo $status_text; ?></span>

            </div>

            <?php if ($is_admin) { ?>

            <form action="<?php echo G5_BBS_URL; ?>/isa_inquiry_status_update.php" method="post" style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">

                <input type="hidden" name="bo_table" value="<?php echo $bo_table; ?>">

                <input type="hidden" name="wr_id" value="<?php echo $wr_id; ?>">

                <button type="submit" name="status" value="견적검토중" class="btn btn_b01">견적검토중</button>

                <button type="submit" name="status" value="견적확인" class="btn btn_b02">견적확인</button>
    ...
    <div id="bo_v_top">
        ...
        <ul class="bo_v_left">
            ...
            <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>" class="btn_b01 btn"><i class="fa fa-pencil-alt" aria-hidden="true"></i> 수정</a></li><?php } ?>


            <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" class="btn_b01 btn" onclick="del(this.href); return false;"><i class="fa fa-trash-alt" aria-hidden="true"></i> 삭제</a></li><?php } ?>
        ...
        <ul class="bo_v_com">
           ...
           <li><a href="<?php echo $list_href ?>" class="btn_b01 btn"><i class="fa fa-list" aria-hidden="true"></i> 목록</a></li>
            ...
            <?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>" class="btn_b02 btn"><i class="fa fa-pencil-alt" aria-hidden="true"></i> 글쓰기</a></li><?php } ?>
    ...
    <?php if ($bo_table == 'inquiry' && $wr_id && $is_admin) { ?>


    <style>


    .estimate-btn-wrap{display:flex;gap:10px;justify-content:center;margin:30px 0;flex-wrap:wrap;}


    .estimate-btn{display:inline-flex;align-items:center;justify-content:center;height:48px;padding:0 25px;border-radius:6px;color:#fff!important;font-size:15px;font-weight:700;text-decoration:none;}


    .estimate-check{background:#2563eb;}


    .estimate-accept{background:#16a34a;}


    .estimate-submit{background:#dc2626;}


    @media(max-width:768px){.estimate-btn-wrap{flex-direction:column;}.estimate-btn{width:100%;}}
    ...
    <div class="estimate-btn-wrap">
        ...
        <a href="/bbs/write.php?w=u&bo_table=inquiry&wr_id=<?php echo $wr_id; ?>&estimate_status=check" class="estimate-btn estimate-check">견적확인</a>


        <a href="/bbs/write.php?w=u&bo_table=inquiry&wr_id=<?php echo $wr_id; ?>&estimate_status=accept" class="estimate-btn estimate-accept">견적접수</a>


        <a href="/bbs/write.php?w=u&bo_table=inquiry&wr_id=<?php echo $wr_id; ?>&estimate_status=submit" class="estimate-btn estimate-submit">견적제출</a>
