    ...
    <?php if (count($list) > 0) { ?>
        ...
        <div class="bp-project-card">
            ...
            <div class="bp-project-actions">
                <a href=G5_ADMIN_URL . "/blog/site_category_mapping.php?site_id=<?php echo (int) $row['id']; ?>" class="btn btn_03">카테고리 매핑</a>
                <a href="./site_form.php?id=<?php echo (int) $row['id']; ?>" class="btn btn_02">수정</a>
                <a href=G5_ADMIN_URL . "/blog/site_delete.php?id=<?php echo (int) $row['id']; ?>&amp;token=<?php echo get_admin_token(); ?>" class="btn btn_01" onclick="return confirm('정말 삭제하시겠습니까?');">삭제</a>
            </div>
        </div>
        <?php } ?>
    <?php } else { ?>
