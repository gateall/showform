...
$short_alias = isset($row['short_alias']) ? trim($row['short_alias']) : '';
...
{
    $size = 210; $cell = 10; $margin = 20; $hash = md5((string)$text);
    $svg = '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 ' . $size . ' ' . $size . '">';
    $svg .= '<rect width="100%" height="100%" fill="#ffffff"/><rect x="0" y="0" width="210" height="210" rx="20" fill="#f8fafc" stroke="#e2e8f0"/>';
    for ($y = 0; $y < 21; $y++) {
        for ($x = 0; $x < 21; $x++) {
            $idx = (($x * 3) + ($y * 5)) % 30;
    ...
    $svg .= '<text x="105" y="198" font-size="10" text-anchor="middle" fill="#64748b">' . htmlspecialchars((string)$text, ENT_QUOTES, 'UTF-8') . '</text></svg>';
    return $svg;
}

function sf_marketing_qr_markup($text)
{
    if (function_exists('qrencode_png_data_uri')) {
        return qrencode_png_data_uri($text);
    }
    if (sf_qr_library_available() && class_exists('QRcode')) {
        ob_start();
...
<div class="sf-wrap">
    <div class="sf-card">
        <h2 class="sf-title">Marketing Tools</h2>
        <div class="sf-muted">Manage landing URL, QR, UTM and short URL in one place.</div>
    </div>

    <div class="sf-grid">
        <div class="sf-card">
            <h3 class="sf-title">Landing URL</h3>
        ...
        <div class="sf-card">
            ...
            <div class="sf-muted" style="margin-top:8px;">
                <?php if (sf_qr_library_available() && class_exists('QRcode')) { ?>QR library detected.<?php } else { ?>No local QR library found. Add `lib/phpqrcode/qrlib.php` to switch automatically.<?php } ?>
            </div>
    ...
    <form method="post" action="<?php echo G5_ADMIN_URL; ?>/landing/marketing_update.php">
        <input type="hidden" name="id" value="<?php echo (int)$row['id']; ?>">
        <div class="sf-card">
            <h3 class="sf-title">Short URL / Alias</h3>
            ...
            <div class="sf-form-grid" style="margin-top:12px;">
                <input type="text" name="short_alias" class="frm_input" placeholder="custom alias" value="<?php echo get_text($short_alias); ?>">
                <input type="text" name="short_url" class="frm_input" value="<?php echo get_text($short_url); ?>">
            ...
            <div class="sf-url-box" style="margin-top:12px;">
                <button type="button" class="sf-btn sf-btn-primary" onclick="sfUpdateUtmUrl()">UTM URL Generate</button>
                ...
                <button type="submit" class="sf-btn sf-btn-light">Save</button>
            </div>
        </div>
    </form>

    <div class="sf-card">
        <h3 class="sf-title">UTM Report</h3>
        <div class="sf-guide">
            <div class="sf-chip"><strong>Source</strong><br><?php echo get_text($utm_source ? $utm_source : 'Not set'); ?></div>
            <div class="sf-chip"><strong>Medium</strong><br><?php echo get_text($utm_medium ? $utm_medium : 'Not set'); ?></div>
            <div class="sf-chip"><strong>Campaign</strong><br><?php echo get_text($utm_campaign ? $utm_campaign : 'Not set'); ?></div>
            <div class="sf-chip"><strong>Report URL</strong><br><?php echo get_text($report_url); ?></div>
        </div>
    </div>

    <div class="sf-card">
        <h3 class="sf-title">Share Tools</h3>
        <div class="sf-url-box">
            <button type="button" class="sf-btn sf-btn-primary" onclick="sfCopyText('<?php echo get_text($landing_url); ?>')">Copy Link</button>
            <button type="button" class="sf-btn sf-btn-light" onclick="alert('Kakao share will be connected later.')">Share to Kakao</button>
            <button type="button" class="sf-btn sf-btn-light" onclick="alert('Facebook share will be connected later.')">Share to Facebook</button>
            <button type="button" class="sf-btn sf-btn-light" onclick="alert('Use the link in Instagram bio or story.')">Share to Instagram</button>
            <button type="button" class="sf-btn sf-btn-light" onclick="alert('Use the link in Threads posts or bio.')">Share to Threads</button>
    ...
    <div class="sf-card">
        <h3 class="sf-title">Promotion Guide</h3>
        <div class="sf-guide">
            <div class="sf-chip"><strong>Naver Place</strong><br>Use for map, reviews and call conversion.</div>
            <div class="sf-chip"><strong>Google Business</strong><br>Use for brand trust and global search visibility.</div>
            <div class="sf-chip"><strong>Naver Power Link</strong><br>Use landing page for search ad conversion.</div>
            <div class="sf-chip"><strong>Google Ads</strong><br>Measure keyword ad conversion with the landing page.</div>
            <div class="sf-chip"><strong>Kakao Channel</strong><br>Use for ?? ?? and repeat visits.</div>
            <div class="sf-chip"><strong>YouTube Shorts</strong><br>Use in video description links.</div>
            <div class="sf-chip"><strong>Instagram Reels</strong><br>Use for visual-heavy industries.</div>
            <div class="sf-chip"><strong>Threads</strong><br>Use for short brand messages.</div>
            <div class="sf-chip"><strong>Facebook</strong><br>Use for local community and retargeting.</div>
        </div>
    </div>
</div>
...
function sfCopyText(text) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(function() { alert('Copied.'); }).catch(function() { sfCopyFallback(text); });
    } else {
        sfCopyFallback(text);
    }
}
