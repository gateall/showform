...
<form id="fcategorylist" method="post" action="<?php echo G5_ADMIN_URL; ?>/landing/category_update.php">
                ...
                <?php if ($total_count > 0) { ?>
                    <?php for ($i = 0; $row = sql_fetch_array($result); $i++) { ?>
                        <tr>
                            ...
                            <td>
                                <a href=G5_ADMIN_URL . "/landing/category_toggle.php?id=<?php echo (int)$row['id']; ?>&amp;token=<?php echo get_admin_token(); ?>" class="btn btn_03"><?php echo $row['is_display'] === 'Y' ? '노출' : '숨김'; ?></a>
                            </td>
                            ...
                            <td>
                                ...
                                <a href=G5_ADMIN_URL . "/landing/category_delete.php?id=<?php echo (int)$row['id']; ?>&amp;token=<?php echo get_admin_token(); ?>" class="btn btn_01" onclick="return confirm('정말 삭제하시겠습니까?');">삭제</a>
                            </td>
                        </tr>
                    <?php } ?>
                <?php } else { ?>
    ...
    <div class="btn_list03 btn_list" style="margin-top:15px;">
        ...
        <button type="submit" formaction="<?php echo G5_ADMIN_URL; ?>/landing/category_delete.php" formmethod="post" onclick="return confirm('선택한 카테고리를 삭제하시겠습니까?');" class="btn btn_01">선택 삭제</button>
    </div>
</form>
